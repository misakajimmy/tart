...
import { BoxLayout, BoxPanel, FocusTracker, Panel, SplitLayout, SplitPanel, TabBar, Widget } from '@lumino/widgets';
...
import { SidePanel, SidePanelHandlerFactory } from './side-panel-handler';
import { StatusBarAlignment, StatusBarImpl } from '../status-bar';
 ...
 */
export const applicationShellLayoutVersion = 5.0;
...
//     }
// }
let ApplicationShell = ApplicationShell_1 = class ApplicationShell extends Widget {
    statusBar;
    ...
    contextKeyService;
    onDidAddWidgetEmitter = new Emitter();
    onDidAddWidget = this.onDidAddWidgetEmitter.event;
    onDidChangeActiveWidgetEmitter = new Emitter();
    onDidChangeActiveWidget = this.onDidChangeActiveWidgetEmitter.event;
    onDidChangeCurrentWidgetEmitter = new Emitter();
    onDidChangeCurrentWidget = this.onDidChangeCurrentWidgetEmitter.event;
    onDidRemoveWidgetEmitter = new Emitter();
    onDidRemoveWidget = this.onDidRemoveWidgetEmitter.event;
    toDisposeOnActiveChanged = new DisposableCollection();
    ...
    statusBar, sidePanelHandlerFactory, applicationStateService, splitPositionHandler, options = {}) {
        super();
  ...
  */
    ...
    createSplitLayout(widgets, stretch, options) {
        let optParam = { renderer: SplitPanel.defaultRenderer, };
        if (options) {
...
ApplicationShell = ApplicationShell_1 = __decorate([
    injectable(),
    __param(0, inject(StatusBarImpl)),
    __param(1, inject(SidePanelHandlerFactory)),
    __param(2, inject(FrontendApplicationStateService)),
    __param(3, inject(SplitPositionHandler)),
    __param(4, inject(ApplicationShellOptions)),
    __param(4, optional())
], ApplicationShell);
...
//# sourceMappingURL=../../../lib/browser/shell/application-shell.js.map
